...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    blob_client = blob_service.get_blob_client(container=safe_container, blob=file_name)

    try:
        properties = blob_client.get_blob_properties()
    except Exception:
        logger.warning("Blob not found. user=%s file=%s", user_id, file_name)
    ...
    if scan_status != "clean":
        logger.warning(
            "Denied SAS issuance. user=%s file=%s scan_status=%s",
            user_id,
            file_name,
            scan_status,
        )
    ...
    sas_url = f"{storage_url}/{safe_container}/{file_name}?{sas_token}"

    logger.info("Issued SAS. user=%s file=%s expiry=%s", user_id, file_name, expiry.isoformat())
